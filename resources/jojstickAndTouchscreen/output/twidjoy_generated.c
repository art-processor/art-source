 ...
 */
/*
 * Handykey Twiddler keyboard as a driver for Linux
 */
...
#define DRIVER_DESC	"Handykey Twiddler keyboard as a driver"
 ...
 * twidjoy. It updates the data accordingly.
 */
 ...
 static void twidjoy_disconnect(struct serio *serio)
{
	...
	input_put_device(twidjoy->dev);
	kfree(twidjoy);
}

/*
 * twidjoy_connect() is the routine that is called when someone adds a
 * new serio device that supports twidjoy protocol and registers it as
 * an input device.
 */
