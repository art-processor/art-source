 ...
 */
/*
 * RC transmitter with 5-byte Zhen Hua protocol joystick driver for Linux
 */
 ...
 * zhenhua. It updates the data accordingly.
 */
 ...
 static void zhenhua_disconnect(struct serio *serio)
{
	...
	input_put_device(zhenhua->dev);
	kfree(zhenhua);
}

/*
 * zhenhua_connect() is the routine that is called when someone adds a
 * new serio device that supports zhenhua protocol and registers it as
 * an input device.
 */
