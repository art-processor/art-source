 ...
 * new serio device that supports Touchright protocol and registers it as
 * an input device.
 */
 ...
 static int touchit213_connect(struct serio *serio, struct serio_driver *drv)
{
	...
	snprintf(touchit213->phys, sizeof(touchit213->phys), %s/input0, serio->phys);
	

	input_dev->name = "Sahara TouchIT-213 Serial TouchScreen";
	...
	input_set_abs_params(touchit213->dev, ABS_X, T213_MIN_XC, T213_MAX_XC, 0, 0);
	input_set_abs_params(touchit213->dev, ABS_Y, T213_MIN_YC, T213_MAX_YC, 0, 0);
