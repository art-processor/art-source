...
MODULE_AUTHOR("Adam Bennett <abennett72@mail.com>");
...
static void hampshire_process_data(struct hampshire *hampshire)
{
	struct input_dev *dev = hampshire->dev;

	if (HAMPSHIRE_FORMAT_LENGTH == ++hampshire->idx) {
		input_report_abs(dev, ABS_X, HAMPSHIRE_GET_XC(hampshire->data));
		input_report_abs(dev, ABS_Y, HAMPSHIRE_GET_YC(hampshire->data));
		input_report_key(dev, BTN_TOUCH,
				 HAMPSHIRE_GET_TOUCHED(hampshire->data));
		input_sync(dev);
...
{
	struct hampshire *hampshire = serio_get_drvdata(serio);

	hampshire->data[hampshire->idx] = data;

	if (HAMPSHIRE_RESPONSE_BEGIN_BYTE & hampshire->data[0])
		hampshire_process_data(hampshire);
	else
...
/*
 * hampshire_disconnect() is the opposite of hampshire_connect()
 */
 ...
 static void hampshire_disconnect(struct serio *serio)
{
	...
	input_get_device(hampshire->dev);
	input_unregister_device(hampshire->dev);
	serio_close(serio);
	serio_set_drvdata(serio, NULL);
	input_put_device(hampshire->dev);
	kfree(hampshire);
}
...
{
	struct hampshire *hampshire;
	struct input_dev *input_dev;
	int err;

	hampshire = kzalloc(sizeof(struct hampshire), GFP_KERNEL);
	input_dev = input_allocate_device();
	if (!hampshire || !input_dev) {
		err = -ENOMEM;
		goto fail1;
	}

	hampshire->serio = serio;
	hampshire->dev = input_dev;
	

	snprintf(hampshire->phys, sizeof(hampshire->phys), %s/input0, serio->phys);
	

	input_dev->name = "Hampshire Serial TouchScreen";
	input_dev->phys = hampshire->phys;
	...
	input_set_abs_params(hampshire->dev, ABS_X, HAMPSHIRE_MIN_XC, HAMPSHIRE_MAX_XC, 0, 0);
	input_set_abs_params(hampshire->dev, ABS_Y, HAMPSHIRE_MIN_YC, HAMPSHIRE_MAX_YC, 0, 0);

	serio_set_drvdata(serio, hampshire);

	err = serio_open(serio, drv);
	if (err)
		goto fail2;

	err = input_register_device(hampshire->dev);
	if (err)
		goto fail3;
